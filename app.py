...
def mycontacts():
    ...
    contact_netids = (db.session.query
    ...
    contacts = {db.session.get(UsersModel, contact[0]) for contact in contact_netids}
    ...
    html_code = flask.render_template("my-contacts.html", user=user, contacts=contacts)
    response = flask.make_response(html_code)
    return response
